import me from "../assets/img/meeeee.png";
import { useState } from "react";

function About (){
     const [showSkills, setShowSkills] = useState(false);
//   const [showExperience, setShowExperience] = useState(false);
  const [showEducation, setShowEducation] = useState(false);
//   const [showMenu, setShowMenu] = useState(false);
    return(
        <>
        <section className=" border border-gray-100 mt-16 bg-gray-100">
            <div className="flex flex-row items-center">
                 <div className="border-1 rounded-full overflow-hidden shadow-[0_0_10px] mt-16 ml-25">
                <img src={me} className="w-70 h-70 object-cover" />
                </div>
                <div className="flex flex-col justify-items">
                <h1 className="ml-10 text-5xl font-medium text-[#283DBA] mt-20">About Me</h1>
                <p className=" ml-10 text-[#3f52c7] mt-3">Hi, I’m Christiana Kobiah — a junior frontend developer with a strong interest in building clean and responsive websites.  </p>
                <p className=" ml-10 text-[#3f52c7]">I’m learning and growing in technologies like HTML, CSS, JavaScript, React, and Tailwind CSS.
  I enjoy turning ideas into real projects and improving my skills every day.</p>

                <p className="ml-10 text-[#3f52c7]">I’m also passionate about tech education and love helping others understand how digital tools work. I'm excited to keep learning and take on new challenges as I grow in the tech space.</p>

                <div className="ml-10 text-white mt-5 text-lg">
                    <button 
                     onClick={() => setShowSkills(!showSkills)}
                    id="skills-btn" className="border rounded border-gray-700 px-3 py-1 bg-[#5f6fda] cursor-pointer "> Skills</button>
                    <button 
                    //  onClick={() => setShowExperience(!showExperience)}
                    id="experience-btn" className="border rounded border-gray-700 px-3 py-1 bg-[#5f6fda] ml-2"> Experience</button>
                    <button
                    onClick={() => setShowEducation(!showEducation)}
                    id="education-btn" className="border rounded border-gray-700 px-3 py-1 bg-[#5f6fda] ml-2 "> Education</button>
                    {/* <button
          onClick={() => setShowMenu(!showMenu)}
          className="bg-purple-600 text-white px-4 py-2 rounded"
        >
          Toggle Menu
        </button> */}
                </div>

                 <div id="skills-section" className={`${showSkills ? '' : 'hidden'} bg-purple-100 p-4 rounded mb-4`}> 
                <p className="ml-10 text-[#5f6fda] font-bold mt-2 mb-4">These are my skills</p>
                <div className="flex flex-row ml-10 ">
                {/* <!-- css Icon --> */}
                 <span>
                    <p className="ml-3 mt-2 text-[#283DBA] font-bold">CSS</p>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-13 w-13 p-2" viewBox="0 0 128 128"><path fill="#5f6fda" d="M18.814 114.123L8.76 1.352h110.48l-10.064 112.754-45.243 12.543-45.119-12.526z"/><path fill="#33A9DC" d="M64.001 117.062l36.559-10.136 8.601-96.354h-45.16v106.49z"/><path fill="#fff" d="M64.001 51.429h18.302l1.264-14.163H64.001V23.435h34.682l-.332 3.711-3.4 38.114h-30.95V51.429z"/><path fill="#EBEBEB" d="M64.083 87.349l-.061.018-15.403-4.159-.985-11.031H33.752l1.937 21.717 28.331 7.863.063-.018v-14.39z"/><path fill="#fff" d="M81.127 64.675l-1.666 18.522-15.426 4.164v14.39l28.354-7.858.208-2.337 2.406-26.881H81.127z"/><path fill="#EBEBEB" d="M64.048 23.435v13.831H30.64l-.277-3.108-.63-7.012-.331-3.711h34.646zm-.047 27.996v13.831H48.792l-.277-3.108-.631-7.012-.33-3.711h16.447z"/></svg>
                </span>
                
                <span>
                     <p className="ml-3 mt-2 text-[#283DBA] font-bold">Tailwindcss</p>
                 {/* Tailwind Icon  */}
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-13 w-13 p-2 ml-4" viewBox="0 0 128 128"><path d="M64.004 25.602c-17.067 0-27.73 8.53-32 25.597 6.398-8.531 13.867-11.73 22.398-9.597 4.871 1.214 8.352 4.746 12.207 8.66C72.883 56.629 80.145 64 96.004 64c17.066 0 27.73-8.531 32-25.602-6.399 8.536-13.867 11.735-22.399 9.602-4.87-1.215-8.347-4.746-12.207-8.66-6.27-6.367-13.53-13.738-29.394-13.738zM32.004 64c-17.066 0-27.73 8.531-32 25.602C6.402 81.066 13.87 77.867 22.402 80c4.871 1.215 8.352 4.746 12.207 8.66 6.274 6.367 13.536 13.738 29.395 13.738 17.066 0 27.73-8.53 32-25.597-6.399 8.531-13.867 11.73-22.399 9.597-4.87-1.214-8.347-4.746-12.207-8.66C55.128 71.371 47.868 64 32.004 64zm0 0" fill="#38bdf8"/></svg>
                 </span>

                 <span>
                     <p className="ml-3 mt-2 text-[#283DBA] font-bold">HTML</p>
                 {/* HTML Icon */}
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-13 w-13 p-2" viewBox="0 0 128 128"><path fill="#E44D26" d="M19.037 113.876L9.032 1.661h109.936l-10.016 112.198-45.019 12.48z"/><path fill="#F16529" d="M64 116.8l36.378-10.086 8.559-95.878H64z"/><path fill="#EBEBEB" d="M64 52.455H45.788L44.53 38.361H64V24.599H29.489l.33 3.692 3.382 37.927H64zm0 35.743l-.061.017-15.327-4.14-.979-10.975H33.816l1.928 21.609 28.193 7.826.063-.017z"/><path fill="#fff" d="M63.952 52.455v13.763h16.947l-1.597 17.849-15.35 4.143v14.319l28.215-7.82.207-2.325 3.234-36.233.335-3.696h-3.708zm0-27.856v13.762h33.244l.276-3.092.628-6.978.329-3.692z"/></svg>
                  </span>
                  </div>

                    <div className="flex flex-row ml-12">
                  {/*  Ract Icon  */}
                   <span>
                     <p className="ml-3 mt-2 text-[#283DBA] font-bold">React</p>
                   <svg xmlns="http://www.w3.org/2000/svg" className="h-13 w-13 p-2" viewBox="0 0 128 128"><g fill="#61DAFB"><circle cx="64" cy="64" r="11.4"/><path d="M107.3 45.2c-2.2-.8-4.5-1.6-6.9-2.3.6-2.4 1.1-4.8 1.5-7.1 2.1-13.2-.2-22.5-6.6-26.1-1.9-1.1-4-1.6-6.4-1.6-7 0-15.9 5.2-24.9 13.9-9-8.7-17.9-13.9-24.9-13.9-2.4 0-4.5.5-6.4 1.6-6.4 3.7-8.7 13-6.6 26.1.4 2.3.9 4.7 1.5 7.1-2.4.7-4.7 1.4-6.9 2.3C8.2 50 1.4 56.6 1.4 64s6.9 14 19.3 18.8c2.2.8 4.5 1.6 6.9 2.3-.6 2.4-1.1 4.8-1.5 7.1-2.1 13.2.2 22.5 6.6 26.1 1.9 1.1 4 1.6 6.4 1.6 7.1 0 16-5.2 24.9-13.9 9 8.7 17.9 13.9 24.9 13.9 2.4 0 4.5-.5 6.4-1.6 6.4-3.7 8.7-13 6.6-26.1-.4-2.3-.9-4.7-1.5-7.1 2.4-.7 4.7-1.4 6.9-2.3 12.5-4.8 19.3-11.4 19.3-18.8s-6.8-14-19.3-18.8zM92.5 14.7c4.1 2.4 5.5 9.8 3.8 20.3-.3 2.1-.8 4.3-1.4 6.6-5.2-1.2-10.7-2-16.5-2.5-3.4-4.8-6.9-9.1-10.4-13 7.4-7.3 14.9-12.3 21-12.3 1.3 0 2.5.3 3.5.9zM81.3 74c-1.8 3.2-3.9 6.4-6.1 9.6-3.7.3-7.4.4-11.2.4-3.9 0-7.6-.1-11.2-.4-2.2-3.2-4.2-6.4-6-9.6-1.9-3.3-3.7-6.7-5.3-10 1.6-3.3 3.4-6.7 5.3-10 1.8-3.2 3.9-6.4 6.1-9.6 3.7-.3 7.4-.4 11.2-.4 3.9 0 7.6.1 11.2.4 2.2 3.2 4.2 6.4 6 9.6 1.9 3.3 3.7 6.7 5.3 10-1.7 3.3-3.4 6.6-5.3 10zm8.3-3.3c1.5 3.5 2.7 6.9 3.8 10.3-3.4.8-7 1.4-10.8 1.9 1.2-1.9 2.5-3.9 3.6-6 1.2-2.1 2.3-4.2 3.4-6.2zM64 97.8c-2.4-2.6-4.7-5.4-6.9-8.3 2.3.1 4.6.2 6.9.2 2.3 0 4.6-.1 6.9-.2-2.2 2.9-4.5 5.7-6.9 8.3zm-18.6-15c-3.8-.5-7.4-1.1-10.8-1.9 1.1-3.3 2.3-6.8 3.8-10.3 1.1 2 2.2 4.1 3.4 6.1 1.2 2.2 2.4 4.1 3.6 6.1zm-7-25.5c-1.5-3.5-2.7-6.9-3.8-10.3 3.4-.8 7-1.4 10.8-1.9-1.2 1.9-2.5 3.9-3.6 6-1.2 2.1-2.3 4.2-3.4 6.2zM64 30.2c2.4 2.6 4.7 5.4 6.9 8.3-2.3-.1-4.6-.2-6.9-.2-2.3 0-4.6.1-6.9.2 2.2-2.9 4.5-5.7 6.9-8.3zm22.2 21l-3.6-6c3.8.5 7.4 1.1 10.8 1.9-1.1 3.3-2.3 6.8-3.8 10.3-1.1-2.1-2.2-4.2-3.4-6.2zM31.7 35c-1.7-10.5-.3-17.9 3.8-20.3 1-.6 2.2-.9 3.5-.9 6 0 13.5 4.9 21 12.3-3.5 3.8-7 8.2-10.4 13-5.8.5-11.3 1.4-16.5 2.5-.6-2.3-1-4.5-1.4-6.6zM7 64c0-4.7 5.7-9.7 15.7-13.4 2-.8 4.2-1.5 6.4-2.1 1.6 5 3.6 10.3 6 15.6-2.4 5.3-4.5 10.5-6 15.5C15.3 75.6 7 69.6 7 64zm28.5 49.3c-4.1-2.4-5.5-9.8-3.8-20.3.3-2.1.8-4.3 1.4-6.6 5.2 1.2 10.7 2 16.5 2.5 3.4 4.8 6.9 9.1 10.4 13-7.4 7.3-14.9 12.3-21 12.3-1.3 0-2.5-.3-3.5-.9zM96.3 93c1.7 10.5.3 17.9-3.8 20.3-1 .6-2.2.9-3.5.9-6 0-13.5-4.9-21-12.3 3.5-3.8 7-8.2 10.4-13 5.8-.5 11.3-1.4 16.5-2.5.6 2.3 1 4.5 1.4 6.6zm9-15.6c-2 .8-4.2 1.5-6.4 2.1-1.6-5-3.6-10.3-6-15.6 2.4-5.3 4.5-10.5 6-15.5 13.8 4 22.1 10 22.1 15.6 0 4.7-5.8 9.7-15.7 13.4z"/></g></svg>
                   </span>

                   <span>
                     <p className="ml-8.5 mt-2 text-[#283DBA] font-bold">JS</p>
                   {/* <!-- Javascript Icon --> */}
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-13 w-13 p-2 ml-4" viewBox="0 0 128 128"><path fill="#F0DB4F" d="M1.408 1.408h125.184v125.185H1.408z"/><path fill="#323330" d="M116.347 96.736c-.917-5.711-4.641-10.508-15.672-14.981-3.832-1.761-8.104-3.022-9.377-5.926-.452-1.69-.512-2.642-.226-3.665.821-3.32 4.784-4.355 7.925-3.403 2.023.678 3.938 2.237 5.093 4.724 5.402-3.498 5.391-3.475 9.163-5.879-1.381-2.141-2.118-3.129-3.022-4.045-3.249-3.629-7.676-5.498-14.756-5.355l-3.688.477c-3.534.893-6.902 2.748-8.877 5.235-5.926 6.724-4.236 18.492 2.975 23.335 7.104 5.332 17.54 6.545 18.873 11.531 1.297 6.104-4.486 8.08-10.234 7.378-4.236-.881-6.592-3.034-9.139-6.949-4.688 2.713-4.688 2.713-9.508 5.485 1.143 2.499 2.344 3.63 4.26 5.795 9.068 9.198 31.76 8.746 35.83-5.176.165-.478 1.261-3.666.38-8.581zM69.462 58.943H57.753l-.048 30.272c0 6.438.333 12.34-.714 14.149-1.713 3.558-6.152 3.117-8.175 2.427-2.059-1.012-3.106-2.451-4.319-4.485-.333-.584-.583-1.036-.667-1.071l-9.52 5.83c1.583 3.249 3.915 6.069 6.902 7.901 4.462 2.678 10.459 3.499 16.731 2.059 4.082-1.189 7.604-3.652 9.448-7.401 2.666-4.915 2.094-10.864 2.07-17.444.06-10.735.001-21.468.001-32.237z"/></svg>
                    </span>

                    <span>
                         <p className="ml-7 mt-2 text-[#283DBA] font-bold">Git</p>
                    {/* <!-- Git/Github Icon --> */}
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-13 w-13 p-2 ml-4" viewBox="0 0 128 128"><path fill="#F34F29" d="M124.737 58.378L69.621 3.264c-3.172-3.174-8.32-3.174-11.497 0L46.68 14.71l14.518 14.518c3.375-1.139 7.243-.375 9.932 2.314 2.703 2.706 3.461 6.607 2.294 9.993l13.992 13.993c3.385-1.167 7.292-.413 9.994 2.295 3.78 3.777 3.78 9.9 0 13.679a9.673 9.673 0 01-13.683 0 9.677 9.677 0 01-2.105-10.521L68.574 47.933l-.002 34.341a9.708 9.708 0 012.559 1.828c3.778 3.777 3.778 9.898 0 13.683-3.779 3.777-9.904 3.777-13.679 0-3.778-3.784-3.778-9.905 0-13.683a9.65 9.65 0 013.167-2.11V47.333a9.581 9.581 0 01-3.167-2.111c-2.862-2.86-3.551-7.06-2.083-10.576L41.056 20.333 3.264 58.123a8.133 8.133 0 000 11.5l55.117 55.114c3.174 3.174 8.32 3.174 11.499 0l54.858-54.858a8.135 8.135 0 00-.001-11.501z"/></svg>
                     </span>
                     </div>

                </div>
                {/* <div id="experience-section" className="bg-purple-100 p-4 rounded mb-4">
                <p className=" text-[#5f6fda] text-lg font-bold mt-2">My experience</p>
                <h1>Junior Frontend Developer </h1>
                <h1 className="mb-3">One year working experience</h1>
                <h1>Android Trainer & Customer Support</h1>
                <h1 className="mb-3">Google Android – Remote | September 2023 – Present</h1>
                <h1>Social Media Marketer & Customer Service Representative</h1>
                <h1 className="mb-3">Lamendy Clothing – Accra, Online | March 2022 – Present</h1>
                <h1>IT Support & Customer Service</h1>
                <h1>Lincoln Community School, Accra | October 2021 – October 2022</h1>
                </div> */}

                <div id="education-section" className={`${showEducation ? '' : 'hidden'} bg-purple-100 p-4 rounded mb-4`}>
                <p className=" text-[#5f6fda] text-lg font-bold mt-2 mr-5">My education</p>
                <h1>University of Cape Coast, Ghana</h1>
                <p>Bachelor’s Degree in Information Technology | 2017 – 2021</p>
                </div>
                </div>
                 {/* <div id="mobile-menu" className={`${showMenu ? '' : 'hidden'} bg-purple-200 p-4 rounded`}>
        
      </div> */}
                
            </div>

        </section>
        </>
    )
}

export default About